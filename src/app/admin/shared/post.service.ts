...
export class PostsService {
  ...
  create(post: Post): Observable<Post> {
    return this.http.post(`${this.baseUrl}posts.json`, post).pipe(
  ...
  getById(id: string): Observable<Post> {
    return this.http.get(`${this.baseUrl}posts/${id}.json`).pipe(
      map((post: Post) => {
        return {
          ...post,
          id,
  ...
  update(post: Post): Observable<Post> {
    return this.http.patch<Post>(`${this.baseUrl}posts/${post.id}.json`, post);
  }
}
